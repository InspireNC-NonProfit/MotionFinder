...
int VideoFileReader::ReadVideoFrameAndSkipDecode(double i)
{
	CheckIfDisposed();
	...
	int bytesDecoded;
	bool exit = false;
	//int duration = data->FormatContext->duration;
	int duration = data->VideoStream->duration;
	duration = duration / 1000;
	//int fps = duration / total;
	int flgs = AVSEEK_FLAG_ANY;
	
	int seek_ts = (i*(data->VideoStream->time_base.den)) / (data->VideoStream->time_base.num);
	//seek_ts = i*FrameRate*1000;

	int ret = libffmpeg::av_seek_frame(data->FormatContext, 1, seek_ts, AVSEEK_FLAG_ANY);
	int wot = FrameCount;
	int wot2 = FrameRate;
	return ret;
